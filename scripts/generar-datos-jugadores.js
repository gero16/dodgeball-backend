...
function generarDatosJugadores() {
  try {
    console.log('📊 Generando datos de jugadores...');
    ...
    const nombresCompletos = jugadoresDatos.map(j => `${j.nombre} ${j.apellido}`);
    ...
    const outputPath = path.join(__dirname, '..', 'jugadores-organizados.json');
    fs.writeFileSync(outputPath, JSON.stringify(jugadoresDatos, null, 2));
    
    console.log(`\n✅ Archivo generado: ${outputPath}`);
    console.log(`📈 Total de jugadores: ${jugadoresDatos.length}`);
    
    // Mostrar lista de jugadores
    console.log('\n📋 Lista de jugadores:');
    jugadoresDatos.forEach((jugador, index) => {
