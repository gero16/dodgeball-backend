...
async function corregirEquiposLocales() {
  try {
    ...
    for (const evento of eventos) {
      ...
      if (participacion.resultados && participacion.resultados.length > 0) {
        participacion.resultados = participacion.resultados.map(resultado => {
          ...
          console.log(`  Resultado: ${resultadoActualizado.equipoLocal} vs ${resultadoActualizado.rival} - ${resultadoActualizado.resultado}`);
      ...
      const resultado = await Evento.findByIdAndUpdate(evento._id, {
