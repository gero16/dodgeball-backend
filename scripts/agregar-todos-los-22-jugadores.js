...
async function agregarTodosLos22Jugadores() {
  try {
    ...
    // 1. Obtener estadísticas existentes
    ...
    console.log(`✅ Encontradas ${estadisticasExistentes.length} estadísticas existentes\n`);
    ...
    for (const jugador of todosLosJugadores) {
      ...
      try {
        // Buscar si ya existe estadística
        const estadisticaExistente = estadisticasExistentes.find(
          (e) =>
            e.jugador.nombre === jugador.nombre &&
            e.jugador.apellido === jugador.apellido
        );
        ...
        if (estadisticaExistente) {
          ...
          console.log(`   ✅ Actualizada - Poder: ${jugador.indicePoder}`);
          estadisticasActualizadas++;
        } else {
          // Crear nueva estadística (asumiendo que el jugador ya existe)
          ...
          await axios.post(`${API_BASE_URL}/estadisticas`, {
            equipo: equipoId,
          ...
          console.log(`   ✅ Creada - Poder: ${jugador.indicePoder}`);
          estadisticasCreadas++;
        }
